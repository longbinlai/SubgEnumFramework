...
public class QtS1Mapper
		extends Mapper<LongWritable, HyperVertexAdjList, HVArraySign, LongWritable> {
	...
	public void map(LongWritable key, HyperVertexAdjList value, Context context) 
		...
		for(int i = 0; i < size - 1; ++i){
			for(int j = i + 1; j < size; ++j){
				v1 = largerThanCur[i];
				v2 = largerThanCur[j];
				if(enableBF && !bloomfilterOpr.get().test(HyperVertex.VertexID(v1), 
						HyperVertex.VertexID(v2))){
					output = false;
				}
				if(output){
					context.write(new HVArraySign(v1, v2, Config.LARGESIGN), key);
				}
			}
		}
	}
