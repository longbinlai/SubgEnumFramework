...
public class HyperGraphStageOneMapper
		extends Mapper<LongWritable, HyperVertexAdjList, HVArray, LongWritable> {
	...
	public void map(LongWritable key, HyperVertexAdjList adjList, Context context) 
		...
		if(largeThanThis.length + smallThanThis.length > thresh){	
			context.write(new HVArray(key.get()), key);
		}
		else{
			context.write(new HVArray(smallThanThis, key.get(), largeThanThis), key);
		}
	}
