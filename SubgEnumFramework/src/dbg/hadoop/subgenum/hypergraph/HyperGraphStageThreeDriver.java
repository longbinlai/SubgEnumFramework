...
public class HyperGraphStageThreeDriver extends Configured implements Tool{

	public int run(String[] args) throws IOException, ClassNotFoundException, InterruptedException, URISyntaxException {
		Configuration conf = getConf();
		// The parameters: <stageOneOutputDir> <stageTwoOutputDir> <outputDir> <numReducers> <jarFile>
		int numReducers = Integer.parseInt(args[3]);
		
		String stageOneOutput = args[0];
		String stageTwoOutput = args[1];
		conf.setBoolean("mapred.compress.map.output", true);
		conf.set("mapred.map.output.compression.codec", "com.hadoop.compression.lzo.LzoCodec");
		
		Job job = new Job(conf, "HyperGraphStageThree");
		((JobConf)job.getConfiguration()).setJar(args[4]);
		//JobConf job = new JobConf(getConf(), this.getClass());
		
		//job.setMapperClass(HyperGraphStageTwoMapper.class);
		job.setReducerClass(HyperGraphStageThreeReducer.class);
		...
		MultipleInputs.addInputPath(job, 
				new Path(stageOneOutput),
				SequenceFileInputFormat.class,
				HyperGraphStageThreeMapper.class);
		
		MultipleInputs.addInputPath(job, 
				new Path(stageTwoOutput),
				SequenceFileInputFormat.class,
				HyperGraphStageThreeMapper.class);
